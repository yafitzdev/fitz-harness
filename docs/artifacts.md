...
5 MB per artifact; larger artifact storage can move behind the same repository contract later.
...
previewed. Text and HTML-like content is inserted with `textContent`, never as markup. PDF is framed as a
same-process blob URL inside an iframe sandboxed with `allow-same-origin` (the renderer CSP allows
`data:`, `blob:`, `https:`, and `http:` in `frame-src`), and media content reaches the renderer as
