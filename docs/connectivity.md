...
`GET /api/v1/connectivity/status` reports normalized Tailscale availability, backend state, MagicDNS
...
unrelated Serve configuration. The commands are only run after an explicit administrator request;
...
fixtures but has not changed or joined a live tailnet.
