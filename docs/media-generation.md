...
- `packages/inference-core/src/scheduler.ts` is a single-GPU FIFO queue (`QueueJob`, leases, cancellation, `queue.updated` events). `packages/inference-core/src/lifecycle-manager.ts` implements on-demand load, leases (active work blocks eviction), idle TTL, and the `ResourceGovernor` VRAM reserve (default 2048 MiB, `packages/inference-core/src/resources.ts`).
...
| KD-9 | **One shared FIFO scheduler** for chat and media jobs. A video job blocks chat on the same GPU (documented policy note in the UI). | Single GPU, single queue; uniform queue position/cancel semantics. Cloud jobs also route through the queue in v1 (see §5.5 trade-off) — including the acknowledged side effect that a cloud media job's lifecycle **recipe-switch** evicts whatever local engine is resident while the no-op cloud instance holds the lease (see §5.5). |
...
Recipe `configuration` gains optional media keys validated by the media adapter: `comfyuiWorkflow` (workflow JSON/graph file), `outputFormats` (`["png","jpg"]`, `["mp4","webm"]`, `["wav","mp3"]`), `defaults` (sampler, steps, guidance, resolution, fps, duration cap), `experimental?: boolean` (hides the recipe from default media-route assignment), and `costCentsPerJob?: number` (credit accounting, §5.9).
...
- **Assignment** stays on the existing `PUT /api/v1/management/routes/:id` handler: it gains validation that the recipe's `modalities.output` includes the route's kind (and rejects `experimental` recipes for media routes unless the admin explicitly assigns them). The modality check applies **only when the request assigns a recipe** (`enabled: true` with a non-empty `recipeId`) — it must never block a de-assignment.
...
`packages/inference-core/src/scheduler.ts` stays the one shared FIFO queue and gains a media entry point:
...
- `QueueJob` becomes a discriminated union (`{ kind: "chat"; request: InferenceRequest; output: AsyncChannel<InferenceDelta> } | { kind: "media"; mediaRequest: MediaGenerationRequest; output: AsyncChannel<MediaJobEvent> }`). The pump (`#pump()`) branches on `job.kind`; queue position, cancellation, `queue.updated` events, and shutdown behavior are unchanged and shared.
...
- **Lease semantics need no policy changes**: `#scheduleEviction()` fires only when `activeLeases === 0` and the instance is READY, so a 5-minute video holds its lease to the terminal state — exactly the existing behavior exercised harder. **Local** media recipes should declare generous idle TTLs (e.g. 300 s) because a busy instance is worth keeping between generation bursts; **cloud** media recipes instead declare `evictionPolicy: "immediate"` (see the next bullet).
...
- **`queue.updated` must not leak media jobs into `inference_requests`**: the host persistence subscriber (`create-app.ts:176–177`) unconditionally calls `store.recordQueueEvent` for every `queue.updated` event, which inserts an `inference_requests` row keyed on `event.data.requestId` (`sqlite-store.ts:291–299`). The `queue.updated` event data therefore gains a **job-kind discriminator** (`kind: "chat" | "media"`), and the subscriber **skips `recordQueueEvent` for media jobs**. Without this, every media job would (a) appear in `GET /api/v1/management/requests` and health recovery counts as a chat-style request, and (b) be double-recovered — `recoverInterruptedRequests` would mark the media copy `interrupted` alongside `recoverInterruptedMediaJobs`. Media queue position remains observable through `queue.updated` for consumers that do not persist to `inference_requests` (metrics, the management status surface, the UI queue panel) and through the job's own `media_job_events` stream (§5.3).
...
submit(request, principal)
  ...
  │            && !(recipe.configuration.experimental && route not explicitly assigned)
...
- **Files/components**: new `apps/host/src/media-jobs.ts` (`MediaJobCoordinator`), `apps/host/src/create-app.ts` (`POST/GET /api/v1/media/jobs…` incl. SSE `/events` replay over `media_job_events`, `ensureMediaRoutes`, startup `recoverInterruptedMediaJobs`, route-kind validation + media **de-assignment** in `parseRoute`/`PUT /routes/:id` — including skipping the handler's recipe-exists guard for empty `recipeId` (create-app.ts:715–716) — `parseRecipe` modalities passthrough, `includeDisabled` management listing, `recordQueueEvent` kind-discriminator guard), `apps/host/src/server.ts` (`reconcileNInferConfiguration` exemption for the well-known media route ids, §5.2).
