...
- Route grants: media routes are **not** auto-granted when a hosted user is created; admins grant `route.use:image` etc. via the existing `PUT /api/v1/management/users/:userId/routes`.
