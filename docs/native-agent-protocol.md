...
The v1 event vocabulary is `run.created`, `run.started`, `assistant.delta`, `run.completed`,
`run.failed`, `run.cancelled`, and `run.interrupted`. Every envelope includes `protocolVersion`,
