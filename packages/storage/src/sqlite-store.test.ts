...
describe("SqliteStore", () => {
  ...
  it("stores artifact content separately from metadata", () => { const store = SqliteStore.memory(); const now = new Date(0).toISOString(); store.createProject({ id: "ap", name: "Artifacts", createdAt: now, updatedAt: now }); store.createSession({ id: "as", projectId: "ap", title: "Artifacts", status: "active", createdAt: now, updatedAt: now }); const artifact = { id: "a1", sessionId: "as", name: "result.txt", mimeType: "text/plain", kind: "text" as const, byteSize: 5, sha256: "hash", createdAt: now, metadata: {} }; store.createArtifact(artifact, Buffer.from("hello")); expect(store.listArtifacts("as")).toEqual([artifact]); expect(Buffer.from(store.getArtifactContent("a1")!).toString()).toBe("hello"); store.close(); });
});
