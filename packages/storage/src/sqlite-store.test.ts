...
describe("SqliteStore", () => {
  ...
  it("persists projects, sessions, canonical transcripts, tool policy, and approvals", () => {
    ...
    expect(second.content.document).toEqual({ version: 1, blocks: [{ id: "markdown:0", type: "markdown", start: 0, end: 2 }] });
  ...
  it("persists standalone chats with their file workspace and keeps them out of project listings", () => { const store = SqliteStore.memory(); const now = new Date(0).toISOString(); store.createProject({ id: "chat-project", name: "Fitz", createdAt: now, updatedAt: now }); store.createSession({ id: "chat-1", workspaceRoot: "C:\\fitz\\chat-workspaces\\chat-1", title: "Standalone", status: "active", createdAt: now, updatedAt: now }); store.createSession({ id: "chat-2", projectId: "chat-project", title: "Wrapped", status: "active", createdAt: now, updatedAt: now }); const chat = store.getSession("chat-1"); expect(chat).toEqual(expect.objectContaining({ title: "Standalone", workspaceRoot: "C:\\fitz\\chat-workspaces\\chat-1" })); expect(chat?.projectId).toBeUndefined(); expect(store.listStandaloneSessions()).toEqual([expect.objectContaining({ id: "chat-1", title: "Standalone", workspaceRoot: "C:\\fitz\\chat-workspaces\\chat-1" })]); expect(store.listSessions("chat-project")).toEqual([expect.objectContaining({ id: "chat-2" })]); store.createSession({ id: "chat-3", title: "Mine", status: "active", ownerUserId: "user-1", createdAt: now, updatedAt: now }); store.createSession({ id: "chat-4", title: "Theirs", status: "active", ownerUserId: "user-2", createdAt: now, updatedAt: now }); expect(store.listStandaloneSessions("user-1").map((session) => session.id)).toEqual(["chat-3"]); store.close(); });
