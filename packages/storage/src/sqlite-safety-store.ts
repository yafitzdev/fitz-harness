...
export class SqliteSafetyStore {
  ...
  listTrashEntriesForRun(runId: string): TrashEntryRecord[] { const rows = this.database.prepare(`SELECT id, run_id, workspace_root, original_path, trash_path, created_at, restored_at FROM trash_entries WHERE run_id = ? ORDER BY created_at, id`).all(runId) as unknown as TrashRow[]; return rows.map(mapTrash); }
