...
interface SessionRow { id: string; project_id: string; owner_user_id: string | null; title: string; status: SessionRecord["status"]; created_at: string; updated_at: string }
...
export class SqliteStore {
  ...
  createSession(session: SessionRecord): void { this.#database.prepare(`INSERT INTO sessions (id, project_id, owner_user_id, title, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)`).run(session.id, session.projectId, session.ownerUserId ?? null, session.title, session.status, session.createdAt, session.updatedAt); }
  getSession(id: string): SessionRecord | undefined { const row = this.#database.prepare(`SELECT id, project_id, owner_user_id, title, status, created_at, updated_at FROM sessions WHERE id = ?`).get(id) as SessionRow | undefined; return row ? mapSession(row) : undefined; }
  listSessions(projectId: string, ownerUserId?: string): SessionRecord[] { const rows = (ownerUserId ? this.#database.prepare(`SELECT id, project_id, owner_user_id, title, status, created_at, updated_at FROM sessions WHERE project_id = ? AND owner_user_id = ? ORDER BY updated_at DESC`).all(projectId, ownerUserId) : this.#database.prepare(`SELECT id, project_id, owner_user_id, title, status, created_at, updated_at FROM sessions WHERE project_id = ? ORDER BY updated_at DESC`).all(projectId)) as unknown as SessionRow[]; return rows.map(mapSession); }
  updateSession(session: SessionRecord): void { this.#database.prepare(`UPDATE sessions SET title = ?, status = ?, updated_at = ? WHERE id = ?`).run(session.title, session.status, session.updatedAt, session.id); }
