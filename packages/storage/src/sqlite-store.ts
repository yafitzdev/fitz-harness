...
export class SqliteStore {
  ...
  /** Artifact content is stored inline as a SQLite BLOB (`artifacts.content`,
   *  migrations v5) with kind-aware size caps enforced at the write boundary
   *  (media-jobs.ts, design doc §5.11). The content-addressed file store
   *  (DESIGN.md §16.1 / media-generation.md OQ-4) will land behind this same
   *  repository contract, so callers stay unaffected. */
