param(
  ...
  [string]$TaskName = "Fitz Harness Host"
)
...
Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $trigger -Settings $settings -Description "Starts the Fitz Harness host at user logon" -Force | Out-Null
