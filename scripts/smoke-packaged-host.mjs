...
const stage = resolve("release/host"); const executable = resolve(stage, "runtime/node.exe"); const server = resolve(stage, "dist/server.js"); const port = await freePort(); const database = resolve(tmpdir(), `fitz-packaged-smoke-${randomUUID()}.db`);
