...
const stage = resolve("release/host"); const executable = resolve(stage, "runtime/node.exe"); const server = resolve(stage, "dist/server.js"); const npmCli = resolve(stage, "node_modules/npm/bin/npm-cli.js"); const piRuntime = resolve(stage, "node_modules/@fitz/agent-pi"); const port = await freePort(); const dataRoot = resolve(tmpdir(), `fitz-packaged-smoke-${randomUUID()}`);
...
const child = spawn(executable, [server], { cwd: stage, windowsHide: true, stdio: "pipe", env: { ...process.env, FITZ_PORT: String(port), FITZ_DATA_ROOT: dataRoot } }); let stderr = ""; child.stderr.on("data", (chunk) => { stderr += String(chunk); });
...
finally { child.kill(); await Promise.race([new Promise((resolveExit) => child.once("exit", resolveExit)), new Promise((resolveDelay) => setTimeout(resolveDelay, 2000))]); if (existsSync(dataRoot)) rmSync(dataRoot, { recursive: true, force: true }); }
