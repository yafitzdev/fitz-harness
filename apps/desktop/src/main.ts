...
}; const template = templates[name]; if (!template) return; const bounds = window.getBounds(); Menu.buildFromTemplate(template).popup({ window, x: bounds.x + clientX, y: bounds.y + clientY }); });
