...
const quota = { maxRequestsPerMinute: 10, maxPromptChars: 200_000, maxOutputTokens: 32_768, maxQueueDepth: 2 };
...
function apiMock() {
  return vi.fn(async (path: string, method?: string) => {
    ...
    if (path.endsWith("/access")) { const user = path.includes("user-2") ? grace : ada; return { data: { user, devices: user === grace ? [{ id: "key-1", name: "Laptop", revokedAt: null }] : [], routeIds: ["audio"], quota, currentDeviceId: "admin-key" } }; }
