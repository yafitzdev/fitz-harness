...
describe("WorkQueueController", () => {
  it("renders both resource lanes and cancels by work id", async () => {
    ...
    await controller.refresh(); expect(count.textContent).toBe("2"); expect(list.textContent).toContain("Current taskGPU · Running"); expect(list.textContent).toContain("image mediaCloud · Position 1");
