...
const item = (id: string, task: string, status: "pending" | "running" | "completed", owner: "main" | "worker", startedAt?: string) => ({
  id, task, status, owner, dependencies: [], workerEligible: owner === "worker", required: true, attempts: 0, ...(startedAt ? { startedAt } : {}),
});
...
describe("AgentPlanPanel", () => {
  ...
  it("spins every running row and previews the earliest-started task", () => {
    const dock = document.createElement("div"); dock.append(document.createElement("form"));
    const panel = new AgentPlanPanel(dock);

    panel.update(plan(1, "active", [
      item("later", "Second task to start", "running", "worker", "2026-08-15T00:00:02Z"),
      item("first", "First task to start", "running", "main", "2026-08-15T00:00:01Z"),
      item("waiting", "Not started", "pending", "main"),
    ]));

    expect(panel.root.querySelectorAll(".agent-plan-panel-item.running .spinner")).toHaveLength(2);
    ...
    expect(panel.root.querySelector(".agent-plan-panel-current-text")?.textContent).toBe("First task to start");
    expect(panel.root.querySelector(".agent-plan-panel-current")?.getAttribute("title")).toBe("First task to start");
  });

  it("keeps the first-seen running task for older plans without timestamps", () => {
    const dock = document.createElement("div"); dock.append(document.createElement("form"));
    const panel = new AgentPlanPanel(dock);
  ...
  it("marks an overflowing collapsed preview for scrolling and a fade", async () => {
    const dock = document.createElement("div"); dock.append(document.createElement("form"));
    const panel = new AgentPlanPanel(dock);
