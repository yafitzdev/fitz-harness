...
export class AgentPlanPanel {
  ...
  #updateCurrentTask(items: AgentPlanItem[]): void {
    ...
    const current = running.reduce<AgentPlanItem | undefined>((earliest, candidate) => {
      if (!earliest) return candidate;
      const earliestStarted = Date.parse(earliest.startedAt ?? "");
      const candidateStarted = Date.parse(candidate.startedAt ?? "");
      if (Number.isFinite(earliestStarted) && Number.isFinite(candidateStarted) && candidateStarted !== earliestStarted) {
        return candidateStarted < earliestStarted ? candidate : earliest;
      }
      return (this.#runningOrder.get(candidate.id) ?? Number.MAX_SAFE_INTEGER) < (this.#runningOrder.get(earliest.id) ?? Number.MAX_SAFE_INTEGER)
        ? candidate
        : earliest;
    }, undefined);
  ...
  #renderItem(item: AgentPlanItem): HTMLLIElement {
    ...
    mark.className = `agent-plan-panel-mark${item.status === "running" ? " spinner" : ""}`;
