...
export class AgentRunCoordinator {
  ...
  start(request: AgentRunRequest, ownerUserId?: string): AgentRunRecord {
    ...
    const now = new Date().toISOString(); const run: AgentRunRecord = { id, routeId: request.model, status: "queued", createdAt: now, updatedAt: now, lastSequence: 0, ...(ownerUserId ? { ownerUserId } : {}) };
  ...
  async #consume(id: string, stream: AgentRuntimeRun | ScheduledStream): Promise<void> {
    ...
    try { for await (const delta of stream) { const event = normalizeRuntimeEvent(delta); this.#emit(id, event.type, event.data); } this.store.updateAgentRun(id, "completed"); this.#emit(id, "run.completed", {}); }
