...
describe("Hosting user administration", () => {
  it("creates a consumer, adds and rotates a key, and removes all access without deleting history", async () => {
    ...
    const created = await app.inject({ method: "POST", url: "/api/v1/management/hosting/users", payload: { displayName: "Grace" } });
    expect(created.statusCode).toBe(201); expect(created.json().data).toEqual({ user: expect.objectContaining({ role: "consumer" }) });
    expect(store.listDevices(created.json().data.user.id)).toEqual([]);
    const issued = await app.inject({ method: "POST", url: `/api/v1/management/users/${created.json().data.user.id}/devices`, payload: { name: "Laptop" } });
    expect(security.authenticate(`Bearer ${issued.json().data.token}`)?.user.id).toBe(created.json().data.user.id);
    const deviceId = issued.json().data.device.id;
    const rotated = await app.inject({ method: "POST", url: `/api/v1/management/devices/${deviceId}/rotate`, payload: {} });
    expect(rotated.statusCode).toBe(201); expect(security.authenticate(`Bearer ${issued.json().data.token}`)).toBeUndefined(); expect(security.authenticate(`Bearer ${rotated.json().data.token}`)?.user.id).toBe(created.json().data.user.id);
    expect(store.listDevices(created.json().data.user.id)).toEqual([expect.objectContaining({ id: rotated.json().data.device.id, name: "Laptop" })]);
    const removed = await app.inject({ method: "DELETE", url: `/api/v1/management/users/${created.json().data.user.id}` });
    expect(removed.statusCode).toBe(200); expect(store.getUser(created.json().data.user.id)?.status).toBe("disabled"); expect(store.listDevices(created.json().data.user.id)).toEqual([]); expect(security.authenticate(`Bearer ${rotated.json().data.token}`)).toBeUndefined();
  });
