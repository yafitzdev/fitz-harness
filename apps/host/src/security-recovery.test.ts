...
describe("host security and recovery boundaries", () => {
  ...
  it("reports and persists interrupted inference and agent work during host restart", async () => {
    ...
    expect(health.json().recovery).toEqual({ interruptedGpuWork: 0, interruptedRequests: 1, interruptedAgentRuns: 1, interruptedToolApprovals: 1, interruptedMediaJobs: 0 });
