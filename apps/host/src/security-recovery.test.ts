...
describe("host security and recovery boundaries", () => {
  ...
  it("reports and persists interrupted inference and agent work during host restart", async () => {
    ...
    store.recordQueueEvent({ sequence: 1, protocolVersion: "1", timestamp: now, type: "queue.updated", data: { requestId: "inference-in-flight", routeId: "fast", kind: "chat", position: 0, depth: 1, status: "started" } });
    ...
    expect(health.json().recovery).toEqual({ interruptedGpuWork: 0, interruptedRequests: 1, interruptedInferenceEvidence: 0, interruptedAgentRuns: 1, interruptedToolApprovals: 1, interruptedMediaJobs: 0 });
